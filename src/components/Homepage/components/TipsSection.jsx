  ...
  return (
    <>
      <div
        className={`${
          isMobile
            ? "block flex-row-reverse px-5 mt-28"
            : "flex px-20 mt-52 space-x-20"
        }`}
      >
        ...
        <div className={`${isMobile && "mt-10"}`}>
          <h1
            className={`${
              isMobile ? "text-[1.3rem]" : "text-[1.8rem] w-[90%]"
            } pb-5 font-bold`}
          >
          ...
          <div className="text-center">
            { !loading && isMobile && renderListTips.length < data.length && (
              <button
                ...
                className="bg-[#798B68] px-5 py-2.5 rounded-sm text-white mt-6"
              >
                Show more
              </button>
            )}
